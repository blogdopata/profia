   <!--  *************** Primer panel ****************** -->

   <a name="contenido"></a>
   <a href="#contenido">.</a>


   <div class="tab-pane fade show active" id="list-home" role="tabpanel" aria-labelledby="list-home-list">
                        <div id="accordionZ">
                        
                          <h1 style="font-family:Roboto Condensed;color:#575758;text-align:center" > Centro de Innovación y Desarrollo de Productos Alimenticios</h1>

                          <div class="card"  >
                            <div  class="card-header bg-autoridades" id="headingOnea">
                              
                                <button class="btn btn-link " data-toggle="collapse" data-target="#collapseOnea" aria-expanded="true" aria-controls="collapseOnea">
                                  Entidad
                                </button>
                             
                            </div>

                            <div id="collapseOnea" class="collapse show" aria-labelledby="headingOnea" data-parent="#accordionZ">
                              <div class="card-body">

                              <img class="img-fluid" src="banner_cidpa.jpg" alt="banner"> <br> <br>

                                <b class="text-fix-content">QUIENES SOMOS</b>
                               
                                <p  class="text-fix-content mt-2">
                                Un factor decisivo en el desarrollo de nuevos productos está en la diferenciación mediante la innovación; de manera que el producto desarrollada no pueda ser fácilmente replicado por la competencia y que al mismo tiempo ofrezca un valor que el consumidor esté dispuesto a pagar. 
                                <br><br>
                                El Centro de Innovación y desarrollo de productos alimenticios (CIDPA) de la Escuela Profesional de Ingeniería Industrial de la Facultad de Ingeniería y Arquitectura de la Universidad San Martin de Porres, cuenta con infraestructura y equipamiento de laboratorios, para la innovación y desarrollo de productos alimenticios, estando a la fecha desarrollando proyectos de investigación aplicada en apoyo a cadenas productivas agroindustriales. 
                                        
                                </p>
                                <br>
                                
                              <div class="text-center mb-4">
                                  
                                  <p class="mt-2 text-fix-content d-flex justify-content-center flex-column"  >
                                    <b  class="text-center" >Coordinador del Centro: Ing. Edy Barnett Mendoza </b>
                                  
                                    <span class="text-center">Investigador CONCYTEC</span> 
                                    <a href="mailto:"  class="text-center " style="color:#585857">Cargo dentro de la Organización: Investigador</a>
                                  </p>
                              </div>

                              <b class="text-fix-content">VISIÓN</b>
                              
                                <p  class="text-fix-content mt-2">
                                Constituirnos en un Centro de Innovación y Desarrollo de productos alimenticios a nivel internacional, conformado con profesionales altamente competitivos.            
                                </p>


                                <b class="text-fix-content mt-5">MISIÓN </b>
                              
                                <p  class="text-fix-content mt-2">
                                Ofrecer servicios de innovación y desarrollo de productos alimenticios en el marco de las nuevas tendencias y necesidades del cliente.   
                                </p>


                                <b class="text-fix-content mt-5">PERSONAL </b>
                              
                                <p  class="text-fix-content mt-2">
                                Contamos con equipo multidisciplinario de profesionales: Ingenieros en Industrias Alimentarias, Químicos, Enólogos, Microbiólogos, nutricionistas y especialista en costeo de productos.
                                </p>
                            </div>
                          </div>
                        </div>

                       


                        <div class="card">
                          <div class="card-header  bg-autoridades" id="headingTwoa">
                            
                              <button class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseTwoa" aria-expanded="false" aria-controls="collapseTwoa">
                                Organización
                              </button>
                           
                          </div>
                          <div id="collapseTwoa" class="collapse" aria-labelledby="headingTwoa" data-parent="#accordionZ">
                            <div class="card-body">
                              <div class="centrarimg">
                                    <img class="img-fluid" src="img/organigrama.jpg" alt="">
                              </div>
                            </div>
                          </div>
                        </div>


                        


                        <div class="card">
                          <div class="card-header  bg-autoridades" id="headingThree">
                          
                              <button class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseThree" aria-expanded="false" aria-controls="collapseThree">
                                Nuestros servicios
                              </button>
                           
                          </div>
                          <div id="collapseThree" class="collapse" aria-labelledby="headingThree" data-parent="#accordionZ">
                            <div class="card-body">
                        

                              <p class="text-fix-content"> 
                              <ul class="fix-items-li">
                            <li>Desarrollo e innovación de productos alimenticios</li>
                            <li>Desarrollo de tecnología</li>
                            <li>Formulación de productos alimenticios</li>
                            <li>Determinación de vida útil</li>
                            <li>Transferencia tecnológica</li>
                            <li>Desarrollo de seminarios y talleres de procesamiento de alimentos</li>
                            <li>Implementación de Línea de Producción</li>
                            <li>Evaluación sensorial </li>
                            <li>Implementación y funcionamiento de laboratorios y plantas piloto</li>
                            <li>Determinar los servicios de mantenimiento o calibración del equipamiento.</li>
                                  </ul>
                            </p>
                            </div>
                          </div>
                        </div>
                      </div>
                            </div>





                            <div class="card">
                          <div class="card-header  bg-autoridades" id="headingSix">
                          
                              <button class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseSix" aria-expanded="false" aria-controls="collapseSix">
                               Laboratorios 
                              </button>
                           
                          </div>
                          <div id="collapseSix" class="collapse" aria-labelledby="headingSix" data-parent="#accordionZ">
                            <div class="card-body">
                        

                              <p class="text-fix-content"> 
                              <b>Laboratorio de Biología y Microbiología</b>
                              <hr>
                              <p>
                                <a style="font-weigth:bold">a)Nombre de la Infraestructura:</a> Laboratorio de Biología y Microbiología <br>
                                <a style="font-weigth:bold">b)Descripción de la infraestructura:</a> Ambiente con ducha de bioseguridad, ambiente adicional de bioseguridad para el cultivo y esterilización de microorganismos. <br>
                                <a style="font-weigth:bold">c)Actividades en las que es usada:</a> Identificacion de microorganismos, análisis microbiológicos de vida útil de alimentos, determinación de calidad microbiológica de alimentos frescos y procesados, estudios de dinámica poblacional en fermentaciones industriales.

                                <div class="centrarimg">
                                <img class="img-fluid img-thumbnail" src="img/lab_1.png" alt="">
                              </div>
                              </p>
                             
                              <br><br>

                              <b class="pt-5">Laboratorio de Físco-Química y Química Orgánica - D3</b>
                              <hr>
                              <p>
                                <a style="font-weigth:bold">a)Nombre de la Infraestructura:</a> Laboratorio de Fisico-Química y Química Orgánica. <br>
                                <a style="font-weigth:bold">b)Descripción de la infraestructura:</a> ambiente con aire acondicionado y extractor de aire, luces de emergencia, proyector multimedia, detector de humo, mesas con luz regulada para los experimentos con instalaciones de gas, agua y desagüe, para el desarrollo de los análisis fisico-químicos con dos ambientes adicionales.  <br>
                                <a style="font-weigth:bold">c)Actividades en las que es usada:</a> Determinacion de parámetros físicos y quimicos en la identificación de moléculas orgánicas en productos alimenticios. 

                                <div class="centrarimg">
                                <img class="img-fluid img-thumbnail" src="img/lab_2.png" alt="">
                              </div>
                              </p>

                              <br><br>


                              <b class="mt-2">Laboratorio de Química Analítica</b>
                              <hr>
                              <p>
                                <a style="font-weigth:bold">a)Nombre de la Infraestructura:</a> Laboratorio de Química Analítica <br>
                                <a style="font-weigth:bold">b)Especificaciones:</a> Ambiente con aire acondicionado y extractor de aire, luces de emergencia, detector de humo, proyector multimedia, mesas con luz regulada para los experimentos con instalaciones de gas, agua y desagüe. <br>
                                <a style="font-weigth:bold">c)Actividades en las que es usada:</a> Identificación de cationes y aniones, análisis espectrofotométricos, análisis fisico-químicos de alimentos.

                                <div class="centrarimg">
                                <img class="img-fluid img-thumbnail" src="img/lab_3.png" alt="">
                              </div>
                              </p>

                              <br><br>


                              <b class="mt-2">Laboratorio de Química Industrial</b>
                              <hr>
                              <p>
                                <a style="font-weigth:bold">a)Nombre de la Infraestructura:</a> Laboratorio de Química Industrial <br>
                                <a style="font-weigth:bold">b)Descripción de la infraestructura:</a> Ambiente con aire acondicionado, proyector multimedia y extintor, luces de emergencia, mesas para los experimentos con instalaciones de agua y desagüe, proyector multimedia y con dispositivos para detectar humos.  con ducha de bioseguridad, ambiente adicional de almacenamiento de reactivos, materiales de vidrio y soluciones.   <br>
                                <a style="font-weigth:bold">c)Actividades en las que es usada:</a> Pruebas fisicoquímicas  

                                <div class="centrarimg">
                                <img class="img-fluid img-thumbnail" src="img/lab_4.png" alt="">
                              </div>
                              </p>


                            </p>
                            </div>
                          </div>
                        </div>










                        
                        <div class="card">
                          <div class="card-header  bg-autoridades" id="headingTen">
                          
                              <button class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseTen" aria-expanded="false" aria-controls="collapseTen">
                               Equipos
                              </button>
                           
                          </div>
                          <div id="collapseTen" class="collapse" aria-labelledby="headingTen" data-parent="#accordionZ">
                            <div class="card-body">
                        
                                        <div class="row">
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_1.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Espectofotómetro</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_2.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Refrigeradora </p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_3.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Sistema de Imagen Molecular</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_4.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Destilador</p>
                                        </div>
                                        </div>

                                       

                                        <div class="row">
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_5.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Incubadora</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_6.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Equipo de Baño María</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_7.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Centrífuga</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_8.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Horno microondas</p>
                                        </div>
                                        </div>




                                        <div class="row">
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_9.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">pH metro </p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_10.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Balanza Analítica</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_11.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Agitador magnético</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_12.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Microscopio</p>
                                        </div>
                                        </div>



                                        <div class="row">
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_13.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Autoclave</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_14.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Cámara de Flujo Laminar</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_15.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Espectrofotómetro UV-Vis Genesys 10S</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_16.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Centrífuga ROTOFIX 32A - HETTICH</p>
                                        </div>
                                        </div>




                                        <div class="row">
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_17.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Centrífuga Gerber</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_18.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Equipo de Baño María JSIB-22T - J&R SAFETY</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_19.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Estufa</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_20.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Mufla</p>
                                        </div>
                                        </div>



                                        <div class="row">
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_21.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Cabina Extractora de Gases</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_22.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Refrigeradora GT – 46HGP marca LG</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_23.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Balanza Analítica</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_24.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Agitador magnético - Thermo Scientific</p>
                                        </div>
                                        </div>



                                        <div class="row">
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_25.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Balanza determinadora de humedad</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_26.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Agitador Vórtex para tubo</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_27.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Selladora al vacío</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_28.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Refractómetro automático digital</p>
                                        </div>
                                        </div>




                                        <div class="row">
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_29.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">pH metro portátil </p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_30.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Horno Microondas Samsung -AGE614ST/XPE</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_31.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Polarímetro</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_32.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Viscosímetro</p>
                                        </div>
                                        </div>






                                        <div class="row">
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_33.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Campana Extractora de Gases </p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_34.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Refractómetro J157 RUDOLPH</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_35.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Balanza de Precisión PL602</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_36.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Cabina Extractora de Gases</p>
                                        </div>
                                        </div>





                                        <div class="row">
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_37.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Estufa - Binder</p>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-6">
                                            <img src="img/eq_38.png" class="img-fluid img-thumbnail">
                                            <p class="text-center">Cocinilla eléctrica</p>
                                        </div>
                                        
                                        </div>

                                        


                              
                            </div>
                          </div>
                        </div>


                        <div class="card">
                          <div class="card-header  bg-autoridades" id="headingElevena">
                            
                              <button class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseElevena" aria-expanded="false" aria-controls="collapseElevena">
                                Líneas de investigación
                              </button>
                           
                          </div>
                          <div id="collapseElevena" class="collapse" aria-labelledby="headingElevena" data-parent="#accordionZ">
                            <div class="card-body text-fix-content">
                              
                                   <p class="text-fix-content" style="font-weight:bold;">
                                     1.- Desarrollo de Proyectos de innovación tecnológica en apoyo a cadenas productivas agroindustriales promisorias
                                   </p>

                                   <ul class="ml-4">
                                     <li>1.1- Cadena Productiva del pisco</li>
                                     <li>1.2.- Cadena productiva de frutas deshidratadas.</li>
                                   </ul>



                                   <p class="mt-4 text-fix-content" style="font-weight:bold;">
                                     2.- Procesamiento de especies agrícolas con potencial agroindustrial.
                                   </p>

                                   <ul class="ml-4">
                                     <li>2.1.-Procesamiento de semillas de eco tipos de sacha inchi Plukenetia volubilis L.</li>
                                     <li>2.2.- Desarrollo de nuevos productos con valor agregado a partir de frutas tropicales y nativas de mayor potencialidad, conforme a las tendencias de mercado.</li>
                                   </ul>
                              
                            </div>
                          </div>
                        </div>
                     


                            <div id="accordion">
                         

                         <div class="card"  >
                           <div  class="card-header bg-autoridades" id="headingFivea">
                             
                               <button class="btn btn-link " data-toggle="collapse" data-target="#collapseFivea" aria-expanded="false" aria-controls="collapseFivea">
                                 Proyectos realizados
                               </button>
                            
                           </div>

                           <div id="collapseFivea" class="collapse" aria-labelledby="headingFivea" data-parent="#accordionZ">
                             <div class="card-body">





<div id="accordionE">
<!-- primero -->
 <div class="card">
   <div class="card-header" id="heading1a" style="background-color:#EDEDED;">
     <h5 class="mb-0">
       <button  style="color:#000000;" class="btn btn-link" data-toggle="collapse" data-target="#collapse1a" aria-expanded="true" aria-controls="collapse1a">
      "Evaluación del uso de conservantes: sorbato de potasio , kilol , acetatos y diacetatos en la vida útil del productocomercial "Sangrecita de pollo".
       </button>
     </h5>
   </div>

   <div id="collapse1a" class="collapse text-fix-content" aria-labelledby="heading1a" data-parent="#accordionE">
     <div class="card-body" style="color:#575858;">
     <b>Objetivos generales </b>
     <p>Evaluar el efecto de conservantes Sorbato de K, Kilol, Acetatos y Diacetatos en la vida útil del producto comercial  “sangrecita de pollo”.</p>
     <b>Objetivos específicos</b>
     <ul class="fix-items-li">
        <li>Determinación de la composición proximal y el porcentaje de hierro de la sangre en estado fresco.</li>
        <li>Determinación de la formulación del producto comercial “sangrecita de pollo”.</li>
        <li>Caracterización físico-química microbiológica y organoléptica del producto elaborado.</li>
        <li>Determinación de la vida útil del producto elaborado</li>
     </ul>
     </div>
   </div>
 </div>
 <!-- fin primero -->


 <div class="card-header"  style="background-color:#EDEDED;">
     <h5 class="mb-0">
       <button  style="color:#000000;" class="btn btn-link">
       “Desarrollo de Módulo automatizado para la determinación de Humedad en Línea en proceso de deshidratación de frutas y hortalizas”.  Prototipo en trámite de Patente.
       </button>
     </h5>
   </div>


   <div class="card-header"  style="background-color:#EDEDED;">
     <h5 class="mb-0">
       <button  style="color:#000000;" class="btn btn-link">
       “Estudio Nutricional y Caracterización Fitoquímica de la Plukenetia huayllabambana sp”, Facultad de Ingeniería y Arquitectura –Facultad de Medicina Humana de la USMP. Proyecto financiado por CONCYTEC-PROCIT 2010. 
       </button>
     </h5>
   </div>


   <div class="card-header"  style="background-color:#EDEDED;">
     <h5 class="mb-0">
       <button  style="color:#000000;" class="btn btn-link">
       “Caracterización química del contenido de ácidos grasos poliinsaturados, fitoesteroles, compuestos fenólicos y actividad antioxidante de 35 ecotipos de semilla de sacha inchi Plukenetia volubilis”, Facultad de Ingeniería y Arquitectura –Facultad de Medicina Humana de la USMP. financiado por  FINCyT-PIBAP 2008.
       </button>
     </h5>
   </div>


   <div class="card-header"  style="background-color:#EDEDED;">
     <h5 class="mb-0">
       <button  style="color:#000000;" class="btn btn-link">
       “Evaluación del contenido de fitoesteroles y compuestos fenólicos y métodos químicos para determinar la actividad antioxidante de la semilla de Sacha inchi Plukenetia volubilis”, Facultad de Ingeniería y Arquitectura –Facultad de Medicina Humana de la USMP financiado por CONCYTEC-PROCIT 2008.
       </button>
     </h5>
   </div>


   <div class="card-header"  style="background-color:#EDEDED;">
     <h5 class="mb-0">
       <button  style="color:#000000;" class="btn btn-link">
       “Caracterización Fitoquimica de ecotipos de caigua Cyclantherapedata, 2008. Facultad de Ingeniería y Arquitectura –Facultad de Medicina Humana de la USMP 2009.
       </button>
     </h5>
   </div>



   <div class="card-header"  style="background-color:#EDEDED;">
     <h5 class="mb-0">
       <button  style="color:#000000;" class="btn btn-link">
       “Desarrollo y Construcción de Prototipo para la Determinación del Grado Alcohólico en Línea en Proceso de Destilación de Pisco”. 2008. Prototipo Patentado.
       </button>
     </h5>
   </div>



   <div class="card-header"  style="background-color:#EDEDED;">
     <h5 class="mb-0">
       <button  style="color:#000000;" class="btn btn-link">
       “Dinámica de la Fermentación y su efecto en la calidad del Pisco”.  USMP.
       </button>
     </h5>
   </div>



   <div class="card-header"  style="background-color:#EDEDED;">
     <h5 class="mb-0">
       <button  style="color:#000000;" class="btn btn-link">
       “Diseño y fabricación de un determinador de porcentaje de volumen de alcohol en línea, usando algoritmo NRW modificado para celda desacoplada, en la obtención del Pisco en las bodegas del Perú”. Proyecto ejecutado con la participación de la Universidad de Pernambuco de Brasil, y financiado por FINCYT-2014.
       </button>
     </h5>
   </div>



   <div class="card-header"  style="background-color:#EDEDED;">
     <h5 class="mb-0">
       <button  style="color:#000000;" class="btn btn-link">
       “Diseño y fabricación de un determinador de porcentaje de volumen de alcohol en línea, usando algoritmo NRW modificado para celda desacoplada, en la obtención del Pisco en las bodegas del Perú”. Proyecto ejecutado con la participación de la Universidad de Pernambuco de Brasil, y financiado por FINCYT-2014.
       </button>
     </h5>
   </div>



   











</div>

                       




                           </div>
                         </div>
                       </div>

                      


                       <div class="card">
                         <div class="card-header  bg-autoridades" id="headingFoura">
                           
                             <button class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseFoura" aria-expanded="false" aria-controls="collapseFoura">
                             Proyectos actuales
                             </button>
                          
                         </div>
                         <div id="collapseFoura" class="collapse" aria-labelledby="headingFoura" data-parent="#accordionZ">
                           <div class="card-body">
                           <div class="container">
                           <div id="accordionD">
<!-- primero -->
 <div class="card">
   <div class="card-header" id="heading4a" style="background-color:#EDEDED;">
     <h5 class="mb-0">
       <button  style="color:#000000;" class="btn btn-link" data-toggle="collapse" data-target="#collapse4a" aria-expanded="true" aria-controls="collapse4a">
      "Determinación de una formulación óptima para la obtención de papas rellenas congeladas utilizando diversas variedades
      de papas nativas peruanas"
       </button>
     </h5>
   </div>

   <div id="collapse4a" class="collapse text-fix-content" aria-labelledby="heading4a" data-parent="#accordionD">
     <div class="card-body" style="color:#575858;">
     <b>Objetivos generales </b>
     <p>
     Obtener papas rellenas con  características estables  de sabor y textura las que habiendo sido expuestas a un proceso de congelación y posterior  almacenamiento durante 4 meses, presente las mismas características de un producto recién procesado.
     </p>
     <b>Objetivos específicos</b>
     <ul class="fix-items-li">
        <li>Determinar una combinación óptima de papas nativas,  que permitan y faciliten un buen moldeado  de la masa, que permitan mantener  su forma y una textura agradable.  </li>
        <li>Se realizan evaluaciones de diferentes formulaciones cuyo producto en la parte central, es una formulación de diversos insumos teniendo como componente principal carne molida, y que constituirá el relleno al interior de la papa.</li>
        <li>Determinar los parámetros de proceso en todas las etapas  de elaboración.</li>
        <li>Para lograr las características  biológicas, físico. Químicas y sensoriales, para la obtención de un producto inocuo y agradable.</li>
     </ul>
     </div>
   </div>
 </div>
 <!-- fin primero -->
<!-- primero -->
<div class="card">
   <div class="card-header" id="heading2a" style="background-color:#EDEDED;">
     <h5 class="mb-0">
       <button  style="color:#000000;" class="btn btn-link" data-toggle="collapse" data-target="#collapse2a" aria-expanded="true" aria-controls="collapse2a">
     "Determinación de los parámetros de deshidratado de lamina flexible edulcorada de frutas (cuero de frutas) hecha a base de beterraga, pitajaya, arándanos y linaza".
       </button>
     </h5>
   </div>

   <div id="collapse2a" class="collapse text-fix-content" aria-labelledby="heading2a" data-parent="#accordionD">
     <div class="card-body" style="color:#575858;">
     <b>Objetivos generales </b>
     <p>
     Determinar los parámetros  de deshidratación para la obtención de la Lámina Flexible edulcorada (Cuero de frutas) a base de una mezcla de las frutas Beterraga (Beta vulgaris L.), pitajaya, (Hylocereus megalanthus L.), Linaza (Linum usitatissium L.) y Arándanos (Vaccinium corymbosum L.) 
     </p>
     <b>Objetivos específicos</b>
     
     <ul class="fix-items-li">
     <li>Determinar la composición proximal del producto.</li>
     <li>Determinar el valor de actividad de agua óptimo.</li>
     <li>Identificar el empaque más adecuado para el almacenamiento de los canutos de lámina flexible de de fruta.</li>
     <li>Determinar la vida útil del producto.</li>
     </ul>
     </div>
   </div>
 </div>
 <!-- fin primero -->
 <!-- primero -->
 <div class="card">
   <div class="card-header" id="heading3a" style="background-color:#EDEDED;">
     <h5 class="mb-0">
       <button  style="color:#000000;" class="btn btn-link" data-toggle="collapse" data-target="#collapse3a" aria-expanded="true" aria-controls="collapse3a">
      "Obtención y caracterización de aguardiente de orujo de uvas pisqueras Quebranta e Italia".
       </button>
     </h5>
   </div>

   <div id="collapse3a" class="collapse text-fix-content" aria-labelledby="heading3a" data-parent="#accordionD">
     <div class="card-body" style="color:#575858;">
     <b>Objetivos generales</b>
     <p>
     Obtención y caracterización de aguardiente de orujo de uvas pisqueras Italia y Quebranta
     </p>
     <b>Objetivos específicos</b>
    <ul class="fix-items-li">
        <li>Identificación de área de producción y adquisición de uvas Italia y Quebranta</li>
        <li>Determinación de parámetros para la obtención de aguardiente de orujo de uvas
pisqueras Italia y quebranta
</li>
        <li>Obtención de aguardiente de uvas pisqueras de uvas Italia y Quebranta a nivel de
Planta Piloto.
</li>
    </ul>
     </div>
   </div>
 </div>
 <!-- fin primero -->


 <div class="card-header" id="heading3a" style="background-color:#EDEDED;">
     <h5 class="mb-0">
       <button  style="color:#000000;" class="btn btn-link">
       "Determinación de componentes volátiles de aguardiente de uva quebranta obtenido haciendo uso de alambique de cobre y de acero inoxidable sometidos a diferentes condiciones de aireación en su periodo de reposo".
       </button>
     </h5>
   </div>









</div>



                             
                           </div>
                      
                      

                   
                         </div>
                       </div>

                       <div class="card">
                          <div class="card-header  bg-autoridades" id="headingSeven">
                            
                              <button class="btn btn-link collapsed" data-toggle="collapse" data-target="#collapseSeven" aria-expanded="false" aria-controls="collapseSeven">
                                Contáctenos
                              </button>
                           
                          </div>
                          <div id="collapseSeven" class="collapse" aria-labelledby="headingSeven" data-parent="#accordionZ">
                            <div class="card-body">
                              <div class="centrarimg">
                              <p class="mt-2 text-fix-content d-flex justify-content-center flex-column"  >
                                    <b  class="text-center" >Coordinador del Centro: Ing. Edy Barnett Mendoza </b>
                                  
                                    <span class="text-center">Investigador CONCYTEC</span> 
                                    <b class="text-center">Celular : 997305859</b>
                                    <span class="text-center">Av. La Fontana 1250. Urb. Santa Patricia. La Molina </span>
                                    <a href="mailto:ebarnettm@usmp.pe"  class="text-center " style="color:#585857">ebarnettm@usmp.pe</a>
                                    
                                  </p>
                              </div>
                            </div>
                          </div>
                        </div>
                  
                       </div>  

                       </div>  